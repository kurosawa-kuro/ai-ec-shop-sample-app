...
- `app/src/pages/Concierge.jsx`: 相談クエリとターン数を state で保持し、追加質問への回答を `元の相談 + 追加質問 + 回答` として再送する2ターン導線を追加。
- `app/src/pages/Concierge.jsx`: followUpQuestion の下に回答候補チップと自由入力フォームを追加。香り/関係性/予算/肌などの質問文から定型候補を出す。
- `app/src/pages/Concierge.jsx`: `相談ターン 1 / 2` 表示を追加し、2ターン目では回答 UI を閉じて「追加条件を反映した提案」と表示。
- `app/src/index.css`: follow-up panel / answer form / turn status のスタイルとモバイル縦積みを追加。
- `app/e2e/demo-flow.spec.js`: 追加質問に「苦手そう」で回答すると2回目の `askConcierge` が走り、回答内容が query に含まれ、再提案が表示されるテストを追加。
- `cd app && npm run build`: passed（Vite の既存 chunk size warning のみ）。
- `cd app && npm run lint`: passed。
- `npx playwright test`: 既存の別 repo dev server が `5173` を占有しているため、この repo を `5175` で起動し一時 config で実行。`18 passed`。
