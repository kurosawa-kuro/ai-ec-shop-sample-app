...
P0 のコンシェルジュ中核4本と P1 は完了済み。次に進めるなら
[refactoring-candidates](../active/refactoring-candidates.md) から整理単位を選ぶ。
...
| 順 | タスク | 何のため |
|---|---|---|
| 5 | [gift-fit-labels](../done/gift-fit-labels.md) | 「失敗しにくい/高見え/香り控えめ」等のラベル（比較の軸データにもなる） |
| 6 | [three-product-comparison-view](../done/three-product-comparison-view.md) | 3品をギフト選定軸で比較して納得 |
| 7 | [carry-consultation-context-to-detail](../done/carry-consultation-context-to-detail.md) | 相談の解決根拠を商品詳細に引き継ぐ／「よく選ばれる理由」 |

## Done: P1 Remaining（体験をさらに強くする）

| 順 | タスク | 何のため |
|---|---|---|
| 8 | [admin-product-gap-detection](../done/admin-product-gap-detection.md) | `/admin-demo` に「商品穴」検出。営業に刺さる店舗インサイト |
| 9 | [ai-gift-message](../done/ai-gift-message.md) | 添え書き生成。購入後押し（華やかさ） |
...
- 2026-07-01 更新: P1 の納得補強3本（ギフト適合ラベル、3品比較、相談文脈の詳細引き継ぎ）を実装・検証済みとして `done/` に移動。残 P1 は店舗インサイト強化と添え書き生成。
- 2026-07-01 更新: 残 P1（商品穴検出、添え書き生成）も実装・検証済みとして `done/` に移動。ロードマップ上の P0/P1 は完了。
